 ...
 */
 document.body.addEventListener('click', function(e) {
    ...
    switch (e.nodeName) {
      case 'CHECKBOX':
        if (e.id === 'origines')
          filteOrigines(articles, tarifs, e.name);
 ...
 */
 function envoyerCommande(panier) {
   let email = prompt("Veuillez entrer une adresse mail", "destinataire@mail.com");
...
function modifierArticle() {
  ...
  for (var i = 1; i < tableau.rows.length-1; i++) {
    if (tableau.rows[i].cells[4].children[0].checked) {
      ...
      resultat = tableau.rows[i].cells[1].children[0].value * tableau.rows[i].cells[2].innerHTML;
      tableau.rows[i].cells[3].innerHTML = resultat.toFixed(2);
      break;
...
function printTicket(panier) {
  ...
  for (var i = 1; i < tableau.children.length-1; i++) {
    pdf.text(150, 240+i*15, tableau.rows[i].cells[0].innerHTML + "          " + tableau.rows[i].cells[1].children[0].value
             + "            " + tableau.rows[i].cells[2].innerHTML + "           " + tableau.rows[i].cells[3].innerHTML);
    nombreArticles++;
  }
  ...
  pdf.text(170, 580, `********************************************`);
  pdf.text("Total : " + panier.total, centrerTicket, 600);
  pdf.text(170, 630, `Merci d'avoir choisi LEGUFRUIT.fr`);
  pdf.text(170, 650, `rue Alsace Forraine, 64040`);
  pdf.text(170, 670, `Pau, France`);
  pdf.text(170, 690, `(+33) 12 34 56 78 90`);
  pdf.text(170, 730, "La date d'achat " + date);
